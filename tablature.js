...
import {
  ...
  getNoteName,
  getNoteOctave,
  isBlackNote as isBlackNotePlaceholder, // see below
  noteToFrequency
} from "./globals.js";
...
/******************************************************
* Because "isBlackNote" was never fully shown in
* older code, here's a quick placeholder:
******************************************************/
function isBlackNote(noteName) {
// e.g., noteName contains '#' => black key
...
// Always pass "synth" as the instrument:
const oscObj= await createOscillator(freq, window.currentInstrument);
...
// stop chord record if needed? - not used, but for sequencer:
