...
let loadedScales = {
    ...
    for (let y = 0; y < numberOfFrets; y++) {
      for (let x = 0; x < numberOfStrings; x++) {
        if (keysState[y][x].marker) {
          const noteName = getNoteName(x, y);
          const octave = getNoteOctave(x, y);
          plainTextNotes.push(`${noteName}${octave} (row=${y}, string=${x})`);
        }
      }
    }
    const now = new Date();
  ...
  function loadSelection(data) {
    if (data.type === "tab") {
      ...
      // Find an empty chord slot or prompt the user
      let targetIndex = chordSlots.findIndex(slot => slot.keys.length === 0);
      if (targetIndex === -1) {
        targetIndex = parseInt(prompt("No empty chord slots. Enter a slot index (1-8):", "1"), 10) - 1;
        if (isNaN(targetIndex) || targetIndex < 0 || targetIndex >= chordSlots.length) {
  ...
  function populateLibrary() {
    ...
    // Get the selected library filter
